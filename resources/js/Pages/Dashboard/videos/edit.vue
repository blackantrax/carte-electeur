<template>
  <AppLayout>
    <PageTitle :title="`Modifier: ${video.title}`" :breadcrumbs="breadcrumbs">
      <template #actions>
        <Link 
...
const initEditor = () => {
  ...
  // Set initial content
  quill.clipboard.dangerouslyPasteHTML(form.value.description || '');
...
const submitVideo = async () => {
  ...
  loading.value = true;
  try {
    await router.put(route('admin.medias.videos.update', { video: props.video.id }), form.value, {
      onSuccess: () => {
