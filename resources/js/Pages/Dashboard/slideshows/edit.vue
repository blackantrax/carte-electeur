<template>
  <AppLayout>
    <PageTitle :title="`Modifier le diaporama: ${slideshow.title}`" :breadcrumbs="breadcrumbs">
      <template #actions>
        <Link 
          :href="route('admin.medias.slideshows.index')" 
              ...
              <div class="flex flex-col sm:flex-row">
                ...
                <div class="w-full sm:w-32 h-32 flex-shrink-0 relative bg-gray-100 dark:bg-gray-700">
                  ...
                  <div class="absolute bottom-0 left-0 right-0 bg-black/50 text-white text-xs px-2 py-1 truncate">
                    {{ image.file?.name || 'Image existante' }}
                  </div>
                  <button 
                    type="button"
...
const breadcrumbs = [
  ...
  { label: 'Diaporamas', url: route('admin.medias.slideshows.index') },
  { label: `Modifier: ${props.slideshow.title}`, active: true }
]
...
const removeImage = (index) => {
  const image = slideshow.value.images[index]
  
  // Libérer l'URL si c'est une nouvelle image
  if (image.preview) {
...
const submitSlideshow = async () => {
  ...
  try {
    ...
    const existingImages = slideshow.value.images
      .filter(img => img.id)
      .map(img => img.id)
    formData.append('existing_images', JSON.stringify(existingImages))

    // Nouvelles images
    slideshow.value.images
      .filter(img => img.file && !img.id)
      .forEach((image, index) => {
        formData.append(`new_images[${index}]`, image.file)
        formData.append(`new_descriptions[${index}]`, image.description)
      })

    // Descriptions mises à jour
    slideshow.value.images.forEach((image, index) => {
      if (image.id) {
        formData.append(`image_descriptions[${image.id}]`, image.description)
      }
    })

    const response = await axios.post(
      route('admin.medias.slideshows.update', { slideshow: slideshow.value.id }),
      formData,
      {
        headers: {
    ...
    toast.success('Diaporama mis à jour avec succès')
    router.visit(route('admin.medias.slideshows.index'))
  } catch (error) {
    console.error(error)
    ...
    if (error.response?.status === 422) {
      ...
      toast.error(error.response?.data?.message || 'Erreur lors de la mise à jour du diaporama')
    }
  } finally {
    loading.value = false
  }
